...
2. Using the credential file you just downloaded, create a Kubernetes Secret and substitute `<keyfile>` with the filename generated by Google for your Google Service Account:
   ...
   kubectl create secret generic workspace-credentials \
       --from-file=workspace-credentials.json=/path/to/<keyfile>.json
3. Edit the file located at [`./google-workspace/workspace-settings.json`](./google-workspace/workspace-settings.json) and fill in correct values for:
