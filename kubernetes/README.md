...
The follow steps only apply if you use Google Workspace as your identity provider. If you use another identity provider, [continue to step 2](#step-2-deploy-1password-scim-bridge-to-the-kubernetes-cluster).
...
On some Kubernetes clusters, health checks can fail for the SCIM bridge before the bridge is able to obtain a Let’s Encrypt certificate. Set `OP_PING_SERVER` to `1` to enable a `/ping` endpoint on port `80` so that health checks will always be brought online:
